...
size_T=floor(a/h);          %number of elements to use along edge of mesh
...
for i=1:length(P_T(:,1))%Index Y
    ...
    for j=1:length(P_T(1,:))%Index X
        if ((i-(length(P_T(:,1))+1)/2)^2+(j-(length(P_T(:,1))+1)/2)^2)<=((length(P_T(:,1))-1)/2)^2% Draw a circle on the square matrix
            P_T(i,j)=1;
            Count=Count+1;
        end
    end
...
for j = 1:length(P_0(1,:))%Index X on focal plane
        Sum = 0;
                ...
                for l = 1:length(P_T(1,:))
                    if P_T(k,l) ~= 0
                        Y_T = ((length(P_T(:,1))+1)/2)-k;%Positon in number of squares not converted to actual distance
                        X_T = -1*(((length(P_T(1,:))+1)/2)-l);%Positon in number of squares not converted to actual distance
                        
                        Y_0 = ((length(P_0(:,1))+1)/2)-i;%Positon in number of squares not converted to actual distance
                        X_0 = -1*(((length(P_0(1,:))+1)/2)-j);%Positon in number of squares not converted to actual distance
