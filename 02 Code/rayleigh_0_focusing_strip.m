...
size_T=100;          %Line along the transducer's diameter
...
size_0 = 100;        %use temporarily for quick computation
P_0 = zeros(size_0,1); %Initialize the pressure pattern on focal plane
...
x = linspace(-6e-3,6e-3,length(P_0));
G = K*a^2/(2*F);

jinc = rho0*c0*u0*G*abs(2*besselj(1,K*a*x/F)./(K*a*x/F));
plot(x*1e3,jinc/(rho0*c0*u0*G),'color','b')
hold on
%plot(x*1e3,abs(P_0)/abs(max(P_0)),'*','color','r')
plot(x*1e3,abs(P_0)/(rho0*c0*u0*G),'*','color','r')
title('Focused circular piston, on axis pressure magnitude','Interpreter','latex')
l = legend('analytical','numerical','Interpreter','latex');
xlabel('$x$ [mm]','interpreter','latex');
ylabel('$ka^2/2d$','interpreter','latex');  % x and y axis labels
axis square
