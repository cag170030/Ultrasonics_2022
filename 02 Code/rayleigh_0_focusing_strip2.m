...
%wrong time convention!!! jinc = sqrt(-1)*K*rho0*c0 *exp(-(sqrt(-1))*K*F)/F .* exp(-sqrt(-1)*K*x.^2/(2*F)).*(a^2/2*u0).*(2*besselj(1,K*a*x/F)./(K*a*x/F)); %complex pressure solution
...
hold on
%plot(x*1e3,abs(P_0)/abs(max(P_0)),'+','color','k') %normalized by max of P_0
...
ylabel('$\rho c_0 u_0 ka^2/2d$','interpreter','latex');  % x and y axis labels
axis square
...
xlabel('$x$ [mm]','interpreter','latex');
ylabel('phase [rad]','interpreter','latex');  % x and y axis labels
axis square
