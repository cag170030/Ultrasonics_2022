...
%size_1 = 51;        %use temporarily for quick computation
            ...
            for k = 1:length(P_T(:,1))
                for l = 1:length(P_T(1,:))
                    if P_T(k,l) ~= 0
                        ...
                        R_1 = sqrt((X_1-X_T)^2+(Y_1-Y_T)^2+z1^2);%Distance between the phase plate and the circular transducer
...
%size_0 = 51;        %use temporarily for quick computation
        ...
        if ((i-(length(P_0(:,1))+1)/2)^2+(j-(length(P_0(1,:))+1)/2)^2)<=(length(P_0(:,1))/2)^2%Draw a circle on the square matrix
            for k = 1:length(P_1(:,1))
                for l = 1:length(P_1(1,:))
                       if P_1(k,l)~=0
>>>>>>> 8da3e9e0ee7dab46b0b71df5b9d58d4147670128
...
<<<<<<< HEAD
 ...
 % No vorticity         Sum = Sum + 1/(2*pi)* S_1(k,l)* P_1(k,l)*(z0-z1)/R0* (-sqrt(-1)*K/R0+1/R0^2)* exp(sqrt(-1)*K*R0);
    
                end
            end
            P_0(i,j)=Sum;%Assign 2nd Rayleigh intergral result onto each point on the focal plane
=======
                       if X_1>=0
                        phi = atan(Y_1/X_1);
                       else
