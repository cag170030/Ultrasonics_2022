...
b=50e-3;%Radius of the circular piston
co=1481;%sound speed in water
...
P_plate=zeros(M_size,M_size);%Initialize the coordinate mesh on the surface of the piston->Then fill in the position of each mesh grid
Count=0;
%P_piston=gpuArray(P_piston);
for i=1:length(P_plate(:,1))%Index Y
    tic
    for j=1:length(P_plate(1,:))%Index X
        if ((i-(length(P_plate(:,1))+1)/2)^2+(j-(length(P_plate(:,1))+1)/2)^2)<=(length(P_plate(:,1))/2)^2% Draw a circle on the square matrix
            P_plate(i,j)=1;
            Count=Count+1;
        end
    end
    time=toc
        waitbar_show(i,P_plate(:,1),time,W_bar)
    
end
imshow(P_plate)% show an imag of the piston
S_piston=P_plate*pi*b^2/Count; %Assigna each mesh square with an area
D_P=10e-2;%Radius of the phase plate
L=23.4e-3;%Distance between the piston and the phase plate
...
for i=1:length(Plate_P(:,1))%Index Y
    ...
    for j=1:length(Plate_P(1,:))%Index X
        ...
        if ((i-(length(Plate_P(:,1))+1)/2)^2+(j-(length(Plate_P(:,1))+1)/2)^2)<=(length(Plate_P(:,1))/2)^2%Draw a circle on the square matrix
            for k=1:length(P_plate(:,1))
                for l=1:length(P_plate(1,:))
                    if P_plate(k,l)~=0
                        Position_pistonY=(length(P_plate(:,1))/2)-k;%Positon in number of squares not converted to actual distance
                        Position_pistonX=-1*((length(P_plate(1,:))/2)-l);%Positon in number of squares not converted to actual distance
                        ...
                        Position_pistonY=Position_pistonY*b/(length(P_plate(:,1))/2);%Covert to Real Y coordinate of the point
                        Position_pistonX=Position_pistonX*b/(length(P_plate(:,1))/2);%Covert to Real X coordinate of the point
...
phi = atan(Position_PlateY/Position_PlateX);
b=75e-3;%Radius of the phase plate
%piecewise definition of phase plate:

for sector = 1:12 % = l = angular momentum number (topological charge)
    Phi = sector*phi;
end
            ...
            for k=1:length(P_piston(:,1))
                for l=1:length(P_piston(1,:))
                    if P_piston(k,l)~=0
                        ...
                        Sum=Sum+1/(2*pi)*pressureresultS_piston(k,l)*exp(-sqrt(-1)*2*pi*f*dist/co)/(2*pi*dist);%Perform rayleigh integral on each mesh point on the piston
                    end
                end
            end
            Plate_P(i,j)=Sum;%Assign rayleigh intergral result onto each point on the phase plate
