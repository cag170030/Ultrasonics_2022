...
title('magnitude incident on phase plate','Interpreter','latex');
axis square
set(gca,'xtick',[],'ytick',[])
...
for j = 1:length(P_0(1,:))%Index X on focal plane
        Sum = 0;
            ...
            for l = 1:length(P_1(1,:)) %Index X on phase plate 
                ...
                %Piecewise-constant phase plate
                %Yuqi:
                %{
                theta1=linspace(pi/6,2*pi,12); %12 increments from pi/6 to 2*pi; note that vorticity is factred into Sum below
                PHI=theta1(ceil(phi/(2*pi/12)));
                %}
                
                %Chirag:
                
                for n = 0:11 % n = sector number
                     if phi >= pi/6*n
                     if phi < pi/6*(n+1)
                        PHI = pi/6*n; 
                     end
                ...
                %Sum = Sum + 1/(2*pi)* S_1(k,l)* P_1(k,l)*exp(sqrt(-1)*3*phi)*(z0-z1)/R0* (-sqrt(-1)*K/R0+1/R0^2)* exp(sqrt(-1)*K*R0);%Perform 2nd Rayleigh integral on each mesh point on the phase plate 
                
                %piecewise phase plate:
                Sum = Sum + 1/(2*pi)* S_1(k,l)* P_1(k,l)*exp(sqrt(-1)*3*PHI)*(z0-z1)/R0* (-sqrt(-1)*K/R0+1/R0^2)* exp(sqrt(-1)*K*R0);%Perform 2nd Rayleigh integral on each mesh point on the phase plate             
            end
            end
